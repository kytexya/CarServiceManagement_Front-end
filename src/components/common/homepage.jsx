...
export default function Homepage() {
    ...
    axios.get(`${baseURL}/api/Location`, {
      ...
      .then((res) => {
        if (res.status === 200) {
          ...
          setDataLocationEnd(locationList.map((item) => ({
            value: item.locationName,
            label: item.locationName
          })));
        }
  ...
  function handleSearch(e) {
    e.preventDefault();
    if (!from || !toLocation || !date) return;
    const routeName = `${from} - ${toLocation}`;
    navigate(`/trip?date=${convertDateFormat(date)}&from=${routeName}&type=earliest`);
  }
        ...
        <form onSubmit={handleSearch}>
          <div className="flex flex-wrap gap-6 px-4 py-2 h-full justify-between">
            <div className="flex flex-col gap-1">
              <p className='text-sm text-gray-500'>Điểm xuất phát</p>
              <select
                required
