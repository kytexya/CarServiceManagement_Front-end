...
export default function AddRouterPage() {
  ...
  useEffect(() => {
    const list = locationList.filter(
      (item) => item.value !== watch("locationStart")
    );
  ...
  useEffect(() => {
    ...
    const locationStartName = locationList.find(
      (item) => item.value === watch("locationStart")
    )?.label;
    const locationEndName = locationEndList.find(
      (item) => item.value === watch("locationEnd")
    )?.label;
    ...
    axios
      ...
      .then((res) => {
        if (res.status === 200) {
          ...
          setDataLocationStart(
            listLocationActive.map((item) => ({
          ...
          <form className="text-sm" onSubmit={handleSubmit(onSubmit)}>
            <div className="flex gap-10 w-full">
              <div className="flex flex-col gap-2 mb-4 w-full">
                ...
                <select
                  className={`border px-5 py-2 rounded-lg ${
                    errors.locationStart ? "border-red-500" : "border-gray"
                  }`}
              ...
              <div className="flex flex-col gap-2 mb-4 w-full">
                ...
                <select
                  className={`border px-5 py-2 rounded-lg ${
                    errors.locationEnd ? "border-red-500" : "border-gray"
                  }`}
                ...
                >
                  <option></option>
                  {locationEndList.length > 0 &&
                    locationEndList?.map((item) => (
                      <option value={item.value}>{item.label}</option>
                    ))}
                </select>
                {errors.locationEnd && (
