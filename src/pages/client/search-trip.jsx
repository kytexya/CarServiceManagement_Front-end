...
export default function SearchTripPage() {
  ...
  const callSearchApi = (date, from, type) => {
    setLoading(true);
    axios.get(`${baseURL}/api/Trip/search?date=${date}&keyword=${from}&type=${type}`, {
      headers: {
...
function TripBox({ data }) {
  ...
  const callApi = () => {
    fetch(`${baseURL}/api/Seat/trip/${data?.tripId}/availability`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        "ngrok-skip-browser-warning": 69420,
        ...
        <div className="flex flex-col w-full">
          ...
          <div className="flex flex-col gap-1">
            <div className="flex flex-row gap-4">
              ...
              <p className="text-sm text-gray-600 font-semibold">Giờ đi: {data?.departureTime}</p>
            </div>
            <div className="flex flex-col gap-1">
              <p className="text-dark font-semibold text-sm">Điểm dừng - Thời gian nghỉ</p>
              {data?.stops?.length > 0 && data?.stops.map((item, index) => (
