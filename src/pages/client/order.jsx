        ...
        {booked === false ?
                ...
                <div className="flex flex-col gap-2 mt-4">
                  ...
                  {tripDetail?.locationRoutes?.length > 0 && tripDetail?.locationRoutes.map((item, index) => (
                    <div key={index} className="flex flex-row gap-1 items-center">
                      ...
                      <p className="text-xs text-gray-600 font-semibold">{item?.stopDuration && item?.stopDuration.slice(0, 5) || '0 phút'}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="flex flex-col md:max-w-fit min-w-[600px] gap-2 border shadow-md rounded-md p-4 h-fit sticky top-22">
            ...
            <div className="flex flex-col gap-2">
              ...
              <button type="button" onClick={handleSubmitTransaction} className="bg-primary text-white font-bold py-2 mt-2 px-4 rounded-md">Xác nhận đã thanh toán</button>
            </div>
          </div>
        }
      </form>
