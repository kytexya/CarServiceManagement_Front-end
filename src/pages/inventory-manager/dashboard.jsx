...
export default function DashboardPage() {
  ...
  const totalInventoryValue = dataList.reduce((sum, item) => sum + (item.stockQuantity * item.price), 0);

  // Calculate monthly imports (mock data)
  const monthlyImports = dataList.filter(item => {
    const importDate = new Date(item.lastImported);
    const currentDate = new Date();
    return importDate.getMonth() === currentDate.getMonth() && 
           importDate.getFullYear() === currentDate.getFullYear();
  }).length;
  ...
  return (
    <div className="flex flex-row w-full">
      ...
      <div className="flex flex-col w-full">
        ...
        <div className="flex justify-between items-center px-8 py-6 border-b bg-white shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Tổng quan nhanh về tình trạng kho phụ tùng</p>
            </div>
          </div>
        ...
        <div className="p-8">
          {/* Alerts */}
          {(lowStockCount > 0 || outOfStockCount > 0 || warrantyWarnings.length > 0) && (
            <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4 mb-6">
            <div className="bg-white rounded-lg border p-4 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{dataList.length}</p>
                </div>
            ...
            <div className="bg-white rounded-lg border p-4 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-600">Nhập tháng này</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div className="bg-white rounded-lg border p-6 shadow-sm">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Phụ tùng sử dụng nhiều nhất</h3>
              <BarChart data={topUsedParts} maxValue={maxUsage} />
            </div>

            <div className="bg-white rounded-lg border p-6 shadow-sm">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Cảnh báo hết hạn bảo hành</h3>
              {warrantyWarnings.length > 0 ? (
                <div className="space-y-3">
                  {warrantyWarnings.map(part => (
                    <div key={part.partId} className="flex items-center gap-3 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                      <WarrantyIcon />
                      <div className="flex-1">
                        <p className="font-medium text-gray-900">{part.partName}</p>
                        <p className="text-sm text-orange-600">Hết hạn: {part.warranty}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p>Không có cảnh báo bảo hành</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg border p-6 shadow-sm mb-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Phụ tùng mới nhập (7 ngày qua)</h3>
            {recentlyImported.length > 0 ? (
              <div className="space-y-3">
                {recentlyImported.map(part => (
                  <div key={part.partId} className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg">
                    <div>
                      <p className="font-medium text-gray-900">{part.partName}</p>
                      <p className="text-sm text-green-600">Nhập: {part.lastImported}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-gray-900">{part.stockQuantity}</p>
              ...
              <div className="text-center py-8 text-gray-500">
                ...
                <p>Không có phụ tùng mới nhập</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-lg border p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <Link to="/inventory-manager/inventory/add" className="flex items-center gap-3 p-4 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 transition-colors">
                <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center">
                  <svg className="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Thêm phụ tùng mới vào kho</p>
                </div>
              </Link>
              <Link to="/inventory-manager/import" className="flex items-center gap-3 p-4 bg-green-50 border border-green-200 rounded-lg hover:bg-green-100 transition-colors">
              ...
              <Link to="/inventory-manager/inventory" className="flex items-center gap-3 p-4 bg-purple-50 border border-purple-200 rounded-lg hover:bg-purple-100 transition-colors">
              ...
              <Link to="/inventory-manager/history" className="flex items-center gap-3 p-4 bg-orange-50 border border-orange-200 rounded-lg hover:bg-orange-100 transition-colors">
